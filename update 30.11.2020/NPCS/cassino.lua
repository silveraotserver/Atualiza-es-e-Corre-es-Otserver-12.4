...
local function creatureSayCallback(cid, type, msg)
	...
	if table.contains({"low", "high", "h", "l", "1", "2", "3", "4", "5", "6", "3x3"}, msg) then
		...
		local dado = math.randomseed(os.time())
		local npcnumber = 0
		...
		elseif table.contains({"3x3"}, msg) then    				-- 3X3 CRIADO POR RENNAN
			for i=1,3 do
				dado = math.random(1, 6)
				thisNpc:say(thisNpc:getName() .. " rolled a ".. dado .. ".", TALKTYPE_MONSTER_SAY)
				npcnumber = npcnumber + dado
			end
			thisNpc:say(thisNpc:getName() .. " npc's number is ".. npcnumber .. ".", TALKTYPE_MONSTER_SAY)
			...
			thisNpc:say(thisNpc:getName() .. " player's number is ".. playernumber .. ".", TALKTYPE_MONSTER_SAY)
				
			if playernumber > npcnumber then
				local wonMoney = bet * (config.bet.win3x3 / 100)
				npcHandler:say("Congratulations, you won! Here's your (".. wonMoney ..") gold coins.", cid)
				config.dicerCounter:sendMagicEffect(math.random(29, 31))
				for _, coin in pairs(createMoney(wonMoney)) do
					Game.createItem(coin[1], coin[2], config.dicerCounter)
			...
			elseif playernumber == npcnumber then
				npcHandler:say("No luck this time. Tied. You won't receive any reward!.", cid)
			else
				npcHandler:say("No luck this time, you lost.", cid)
			end
		end
...
npcHandler:setMessage(MESSAGE_GREET, "Welcome to my cassino! I'm offering {high/low}, {3x3} and {numbers}, to start just put your {money} on {counter} and say {number} or {high/low} or {3x3}.")
