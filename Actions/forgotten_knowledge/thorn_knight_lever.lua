...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		...
		if player:getExhaustion(Storage.ForgottenKnowledge.ThornKnightTimer) > 0 then
		...
		for y = 32877, 32881 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getExhaustion(Storage.ForgottenKnowledge.ThornKnightTimer) >= 1 then
		...
		for y = 32877, 32881 do
			...
			if playerTile and playerTile:isPlayer() then
				if verificarStorage < 1 then
					...
					playerTile:setExhaustion(Storage.ForgottenKnowledge.ThornKnightTimer, 6.0 * 60.0 * 60.0)
				end
			end
		end		
