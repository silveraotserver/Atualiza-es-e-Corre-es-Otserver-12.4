...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		...
		if player:getExhaustion(Storage.ForgottenKnowledge.LloydTimer) > 0 then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You recently challenged Lloyd. You can challenge him again in 6 hours.")
			return true
		end
		...
		for y = 32868, 32872 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getExhaustion(Storage.ForgottenKnowledge.LloydTimer) >= 1 then
		...
		for y = 32868, 32872 do
			...
			if playerTile and playerTile:isPlayer() then
				if verificarStorage < 1 then
					...
					playerTile:setExhaustion(Storage.ForgottenKnowledge.LloydTimer, 6.0 * 60.0 * 60.0)
				end
