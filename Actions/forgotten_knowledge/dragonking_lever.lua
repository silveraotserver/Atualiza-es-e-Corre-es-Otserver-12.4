...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		...
		if player:getExhaustion(Storage.ForgottenKnowledge.DragonkingTimer) > 0 then
		...
		for y = 31178, 31182 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getExhaustion(Storage.Dragonking.DragonkingTimer) >= 1 then
		...
		for y = 31178, 31182 do
			...
			if playerTile and playerTile:isPlayer() then
				if verificarStorage < 1 then
					...
					playerTile:setExhaustion(Storage.ForgottenKnowledge.DragonkingTimer, 6.0 * 60.0 * 60.0)
				end
